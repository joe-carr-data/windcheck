...
> **This is the audit record, preserved as filed.** It measures published
> meshes and reports what it finds; it does not transform anything. The work
> that followed — bounded displacement, certified excision, and reload-verified
> transverse-clean outputs for the same corpus — is a separate layer described
> in [`../README.md`](../README.md) and [`CORPUS.md`](CORPUS.md), and released
> as `v0.1.0-beta`. The per-segment results this page cites are in
> [`../results/`](../results). Where the two layers state a figure differently,
> the difference is one of population rather than of measurement: this page
> counts crossings in the **published** meshes, and the release counts them in
> the meshes it emits.
