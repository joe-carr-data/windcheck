...
\textbf{Both controls pass, and one of them fires.} The null control is not
rhetorical: 44 labelled single-wrap segments return 0.00\%, and a unit test
holds that property so it cannot silently regress. The validity filter is not
...
\textbf{No existing tool does this.} A search of the
\texttt{volume-cartographer} tree finds no self-contact, injectivity or
fold check anywhere on the mesh representation; the one relevant parameter,
\texttt{resume\_local\_row\_self\_intersection\_cell\_factor}, has no consumer
in the codebase. The naive alternative --- nearest-\emph{sample} distance ---
...
0.06 percentage points (6.60\% versus 6.54\% inside 2.0 voxels), with the
